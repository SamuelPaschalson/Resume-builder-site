...
if (isset($_POST['work-d'])) {
  $ids = $_POST['id'];
    header("location: work-details?id=".$ids."&ids=1");
}
...
if(isset($_POST['work_next'])) {
    ...
    $social_arr = json_decode($social,true);
    $contact1 = file_get_contents('contactdata.json');
    $contact_arr1 = json_decode($contact1,true);
    $social1 = file_get_contents('social.json');
    $social_arr1 = json_decode($social1,true);
    $flag = false;
    $flag1 = false;
    echo("Ok");
    foreach ($contact_arr as $key => $value)
      ...
      foreach ($social_arr as $key1 => $value1)
      {
        if ($value1['id'] == $id) 
          {
                    $flag1 = true;
  ...
  if (!$flag || !$flag1) {
    if ($_FILES["image"]['name'] == null){
            $contact_arr1[] = array('id'=>$id, 'firstname'=>$fname, 'lastname'=>$lname, 'age'=>$age, 'profession'=>$profession, 'city'=>$city, 'state'=>$state, 'country'=>$country, 'postal'=>$postal, 'phone'=>$phone, 'email'=>$email, 'image'=>null);
            $social_arr1[] = array('id'=>$id, 'facebook'=>$fb, 'instagram'=>$ig, 'github'=>$git, 'linkedin'=>$link);
    } else{
        ...
        $contact_arr1[] = array('id'=>$id, 'firstname'=>$fname, 'lastname'=>$lname, 'age'=>$age, 'profession'=>$profession, 'city'=>$city, 'state'=>$state, 'country'=>$country, 'postal'=>$postal, 'phone'=>$phone, 'email'=>$email, 'image'=>$img);
        $social_arr1[] = array('id'=>$id, 'facebook'=>$fb, 'instagram'=>$ig, 'github'=>$git, 'linkedin'=>$link);
      }
      file_put_contents('contactdata.json', json_encode($contact_arr1, JSON_PRETTY_PRINT));
      file_put_contents('social.json', json_encode($social_arr1, JSON_PRETTY_PRINT));
        $ids['uniqid'] = $id;
              header("location: work-history?id=".$ids['uniqid']."&ids=1");
  }
  if ($flag || $flag1) {
    if ($_FILES["image"]['name'] == null){
      $img == null;
      ...
      $social_arr[$key]["facebook"] = $fb;
      $social_arr[$key]["instagram"] = $ig;
      $social_arr[$key]["github"] = $git;
      $social_arr[$key]["linkedin"] = $link;
      $ids['uniqid'] = $id;
      $arry = json_encode($contact_arr, JSON_PRETTY_PRINT);
      $arry1 = json_encode($social_arr, JSON_PRETTY_PRINT);
      file_put_contents('contactdata.json', $arry);
      file_put_contents('social.json', $arry1);
         header("location: work-history?id=".$ids['uniqid']."&ids=1");
  }
...
if(isset($_POST['workh'])) {
    ...
    if (!$flag) {
      ...
      file_put_contents('workhistory.json', json_encode($workh_arr1, JSON_PRETTY_PRINT));
        $ids['uniqid'] = $id;
          header("location: work-details?id=".$ids['uniqid']."&ids=$idd");
    }
    if ($flag) {
      $workh_arr[$key]["id"] = $id;
    ...
    foreach ($workh_arr as $key => $value )
      ...
      if (!$flag) {
        ...
        file_put_contents('workdetails.json', json_encode($workh_arr1, JSON_PRETTY_PRINT));
          $ids['uniqid'] = $id;
            header("location: check?id=".$ids['uniqid']."&ids=$idd");
      }
      if ($flag) {
        $workh_arr[$key]["id"] = $id;
        $workh_arr[$key]["ids"] = $idd;
...
if (isset($_POST['check'])) {
  ...
  if ($check == 'Yes') {
    ...
    echo 'yeh';
    echo $idss;
    header("location: work-history?id=$id&ids=$idss");
  }
  if ($check == 'No') {
    header("location: education?id=$id&ids=1");
  }
}
...
if (isset($_POST['check1'])) {
  ...
  if ($check == 'Yes') {
    ...
    echo 'yeh';
    echo $idss;
    header("location: education?id=$id&ids=$idss");
  }
  if ($check == 'No') {
    header("location: certification?id=$id&ids=1");
  }
}
 ...
 if(isset($_POST['edu'])) {
    ...
    if (!$flag) {
      ...
      file_put_contents('edudata.json', json_encode($workh_arr1, JSON_PRETTY_PRINT));
        $ids['uniqid'] = $id;
          header("location: check1?id=".$ids['uniqid']."&ids=$idd");
    }
    if ($flag) {
      $workh_arr[$key]["id"] = $id;
      $workh_arr[$key]["ids"] = $idd;
